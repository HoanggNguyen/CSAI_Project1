...
def createMatrix(file_name):
    with open(file_name, 'r') as file:
        # Đọc dòng đầu tiên và chuyển thành list trọng số
        ...
        # Đọc từng dòng còn lại và xử lý trực tiếp
        for row_index, line in enumerate(file):
            row = list(line.strip())  # Loại bỏ '\n' bằng strip()
            matrix.append(row)

            # Duyệt qua từng phần tử của dòng để cập nhật stones, ares_pos, switches
...
def output(sc,algo, input_path, output_path):
    ...
    with open(output_path, 'w') as file:
        if result is not None:
            ...
            # In ra màn hình
            print(algo.__name__)
            print(f'Steps: {len(steps)}, Weight: {weight}, Node: {node}, Time (ms): {total_time:.2f}, Memory (MB): {usage:.2f}')
            print(steps)

            # Ghi ra tệp
            file.write(f"{algo.__name__}\n")
            file.write(f"Steps: {len(steps)}, Weight: {weight}, Node: {node}, Time (ms): {total_time:.2f}, Memory (MB): {usage:.2f}\n")
            file.write(steps + "\n")
        else:
